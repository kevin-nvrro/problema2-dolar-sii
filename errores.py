...
def redondear_2_cifras(valores):
    # aqui redondeamos a 2 cifras
    magnitud = np.floor(np.log10(np.abs(valores)))
    factor = 10 ** (magnitud - 1)

    # aqui se redondea al valor más cercano
...
def ej_compra_venta(monto, p_compra, er_compra, p_venta, er_venta):
    ...
    # el error lo pasamos a absoluto y calculamos ganancia
    ea_pesos_final = pesos_final * (er_pesos_final / 100)
    ganancia = pesos_final - monto

    # son iguales porque se ocupa un valor exacto y finalmente se calcula el e.relativo de la ganancia
...
def redondear_3_cifras(valores):
    # aqui es lo mismo que antes pero restamos 2
    magnitud = np.floor(np.log10(np.abs(valores)))
...
if __name__ == '__main__':
    ...
    ganancia, error_abs_ganancia, error_rel_ganancia = ej_compra_venta(monto, precio_compra, err_rel_compra, precio_venta, err_rel_venta)
    ...
    # sacamos el indice del minimo y maximo
    ...
    # calculamos rentabilidad y su error absoluto porcentual
